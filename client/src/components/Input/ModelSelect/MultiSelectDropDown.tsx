        ...
        <Listbox value={value} onChange={handleSelect} disabled={disabled}>
          {() => (
            <>
              <ListboxButton
                className={cn(
                  'relative flex w-full cursor-default flex-col rounded-md border border-black/10 bg-white py-2 pl-3 pr-10 text-left focus:outline-none focus:ring-0 focus:ring-offset-0 dark:border-border-medium dark:border-white/20 dark:bg-surface-secondary sm:text-sm',
                  className ?? '',
                )}
                id={excludeIds[0]}
              ...
              >
                ...
                {showLabel && (
                  <Label
                    className={cn('block text-xs text-text-secondary dark:text-text-tertiary', labelClassName)}
                    id={excludeIds[1]}
                ...
                <span className="inline-flex w-full truncate" id={excludeIds[2]}>
                  <span
                    className={cn(
                      'flex h-6 items-center gap-1 truncate text-sm text-text-primary dark:text-white',
                      !showLabel ? 'text-xs' : '',
                    )}
                  >
              ...
              >
                <ListboxOptions
                  ref={menuRef}
                  className={cn(
                    'absolute z-50 mt-2 max-h-60 w-full overflow-auto rounded bg-white text-base text-xs ring-1 ring-black/10 focus:outline-none dark:bg-surface-secondary dark:ring-white/20 dark:last:border-0 md:w-[100%]',
                    optionsClassName,
                  )}
                >
                  {searchRender}
                  {options.map((option, i: number) => {
                    ...
                    return (
                      <ListboxOption
                        ...
                        className="group relative flex h-[42px] cursor-pointer select-none items-center overflow-hidden border-b border-black/10 pl-3 pr-9 text-text-primary last:border-0 hover:bg-gray-20 dark:border-white/20 dark:text-white dark:hover:bg-surface-secondary"
                      >
                        <span className="flex items-center gap-1.5 truncate">
                          ...
                          <span
                            className={cn(
                              'flex h-6 items-center gap-1 text-text-primary dark:text-text-primary',
                          ...
                          {option.isButton && (
                            <span className="absolute inset-y-0 right-0 flex items-center pr-3 text-text-primary dark:text-text-primary">
                              <ArrowRight />
                            </span>
                          )}
