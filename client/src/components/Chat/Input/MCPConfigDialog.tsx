    ...
    <OGDialog open={isOpen} onOpenChange={onOpenChange}>
      <OGDialogTemplate
          ...
          <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4 px-6 pb-2">
            {Object.entries(fieldsSchema).map(([key, details]) => (
              <div key={key} className="space-y-2">
                ...
                {details.description && (
                  <p
                    className="text-xs text-text-secondary [&_a]:text-brand [&_a]:hover:text-brand dark:[&_a]:text-brand dark:[&_a]:hover:opacity-80"
        ...
        selection={{
          ...
          selectClasses: 'bg-brand hover:opacity-90 text-white',
